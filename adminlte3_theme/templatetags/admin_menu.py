    ...
    def render(self, context, menus=None):
        ...
        for group in menus:
            ...
            if len(group[key]['children']) > 0:
                r += '<a href="#" class="nav-link">%s <span>%s</span><span class="pull-right-container"><i class="fas fa-angle-left right"></i></span></a><ul class="treeview-menu">\n' % (
                ...
                if (request.path == reverse('admin:index')):
                    r += '<a href="%s" class="nav-link">%s <p>%s</p></a>\n' % (
    ...
    def admin_apps(self, context, r):
        ...
        for app in context['available_apps']:
            ...
            for model in app['models']:
                ...
                if (request.path == url):
                    r += '<a href="%s" class="nav-link active">%s %s</a>' % (
                        url, icon, model['name'])
                else:
                    r += '<a href="%s" class="nav-link">%s %s</a>' % (
                        url, icon, model['name'])
